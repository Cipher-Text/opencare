...
import { Checkbox, Button, Typography, Menu } from 'antd';
        ...
        <div style={{
            ...
            <Menu mode="inline" defaultSelectedKeys={['1']} defaultOpenKeys={['sub1']}>
                ...
                <Menu.SubMenu key="sub1" title={<span>{title}</span>}>
                    {/* Render checkboxes */}
                    {items.slice(0, showAll ? items.length : maxDisplayCount).map((item, index) => (
                        <Menu.Item key={index}>
                    ...
                    {items.length > maxDisplayCount && (
                        <Menu.Item key="showMoreLess" onClick={toggleShowAll}>
                            {showAll ? (
